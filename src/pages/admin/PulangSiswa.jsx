...
import {
  ...
  FileSpreadsheet, Hash, QrCode, Fingerprint, TrendingDown,
  TrendingUp, Minus, BarChart3, ScanLine, X, Loader, GraduationCap
} from 'lucide-react'
...
const fmtSelisih = (menit) => {
  ...
  const dur = jam > 0 ? (sisa > 0 ? `${jam} jam ${sisa} menit` : `${jam} jam`) : `${abs} menit`
  if (menit > 0) return <span className="text-amber-600 dark:text-amber-400 font-semibold">-{dur} lebih awal</span>
  if (menit < 0) return <span className="text-emerald-600 dark:text-emerald-400 font-semibold">+{dur} lembur</span>
...
const statusPulangBadge = (s, menit) => {
  ...
  const cfg = {
    ...
    lembur: { cls:'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300', lbl:`🌙 Lembur ${dur}` },
...
export default function PulangSiswa() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await adminApi.getAbsensiSiswaPulang(params)
  ...
  const fetchStatistik = async () => {
    try {
      const res = await adminApi.getAbsensiSiswaPulangStatistik({ bulan: filters.bulan, tahun: filters.tahun })
  ...
  const handleAbsenManual = async (e) => {
    e.preventDefault()
    if (!nisnInput.trim()) { toast.error('Masukkan NIS/NISN siswa'); return }
    setAbsenLoading(true); setAbsenResult(null)
    try {
      const res = await adminApi.absenSiswaPulangManual({ nisn: nisnInput.trim() })
      const d   = res.data.data
  ...
  const startScanner = async () => {
    setShowScannerPulang(true)
    await new Promise(r => setTimeout(r, 200))
    try {
      const qr = new Html5Qrcode('qr-pulang-siswa')
      html5QrRef.current = qr
      await qr.start({ facingMode:'environment' }, { fps:10, qrbox:250 }, async (decoded) => {
        await stopScanner()
        setAbsenLoading(true); setAbsenResult(null)
        try {
          const res = await adminApi.absenSiswaPulangQr({ qr_code: decoded })
          const d   = res.data.data
          setAbsenResult({ success: true, data: d, message: res.data.message })
          toast.success(res.data.message)
          fetchData(); fetchStatistik()
    ...
    } catch { setShowScannerPulang(false) }
  }
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await adminApi.exportAbsensiSiswaPulang({ bulan: filters.bulan, tahun: filters.tahun })
      const url = URL.createObjectURL(new Blob([res.data]))
      const a   = document.createElement('a')
      a.href = url; a.download = `Absensi-Pulang-Siswa-${filters.bulan}-${filters.tahun}.xlsx`
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="relative flex-shrink-0">
            {row.siswa.foto
              ...
              : <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-bold text-sm shadow">{row.siswa.nama.charAt(0)}</div>
            }
    ...
    <div className="w-full max-w-full overflow-x-hidden space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl px-4 py-3.5">
        <motion.div initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-800 dark:text-slate-100">Absensi Pulang Siswa</h1>
            <p className="text-[11px] text-slate-400 mt-0.5 flex items-center gap-1">
              <Sparkles size={10} className="text-emerald-500"/>
              Jam pulang sekolah: <span className="font-bold text-emerald-600 dark:text-emerald-400 ml-1">{jamPulangSet}</span>
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleRefresh} disabled={refreshing}
            ...
            <RefreshCw size={15} className={refreshing ? 'animate-spin text-emerald-500' : 'text-slate-500'}/>
          </button>
          <button onClick={handleExport}
            className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl text-xs font-bold shadow-lg shadow-emerald-500/25 hover:opacity-90 transition-all">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800/60 rounded-xl w-fit">
        {[{key:'list',label:'Daftar Pulang'},{key:'statistik',label:'Statistik'}].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${tab===t.key ? 'bg-white dark:bg-slate-800 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {statistik && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            {label:'Lembur',       val:statistik.total_lembur,          icon:TrendingUp,  color:'teal',   sub:`Rata ${statistik.rata_lembur} mnt`},
          ].map((s,i) => {
            const cls = {
              ...
              teal:   {bg:'bg-teal-50 dark:bg-teal-900/20',     border:'border-teal-200 dark:border-teal-800',     text:'text-teal-600 dark:text-teal-400',     icon:'bg-teal-100 dark:bg-teal-900/40'},
            }[s.color]
            return (
        ...
        {tab === 'list' && (
          <motion.div key="list" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/10 dark:to-teal-900/10">
                <div className="w-7 h-7 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center">
                  <LogOut size={14} className="text-emerald-600 dark:text-emerald-400"/>
                </div>
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100">Input Absen Pulang Siswa</p>
                  <p className="text-[11px] text-slate-400">Manual (NIS/NISN) atau scan QR Code siswa</p>
                </div>
              </div>
              <div className="p-5">
                <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl w-fit mb-4">
                  {[{key:'manual',label:'✍️ Manual NIS'},{key:'qr',label:'📷 QR Code'}].map(m => (
                    <button key={m.key} onClick={() => { setAbsenMode(m.key); if(showScannerPulang) stopScanner(); setAbsenResult(null) }}
                      className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${absenMode===m.key ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                      {m.label}
                    </button>
                  ))}
                </div>
                ...
                {absenMode === 'manual' ? (
                  <form onSubmit={handleAbsenManual} className="flex gap-2 flex-wrap">
                    <div className="relative flex-1 min-w-[200px]">
                      ...
                      <input type="text" value={nisnInput} onChange={e => setNisnInput(e.target.value)}
                        placeholder="Masukkan NIS atau NISN siswa..."
                        className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-slate-900 dark:text-white placeholder-slate-400 font-mono"/>
                    </div>
                    <button type="submit" disabled={absenLoading}
                      className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/25 hover:opacity-90 disabled:opacity-60 transition-all">
                  ...
                  <div className="space-y-3">
                    {!showScannerPulang ? (
                      <button onClick={startScanner}
                        className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/25 hover:opacity-90 transition-all">
                        <ScanLine size={14}/> Mulai Scan QR
                      </button>
                    ) : (
                      <div className="space-y-2">
                        <div id="qr-pulang-siswa" className="w-full max-w-xs rounded-2xl overflow-hidden border-2 border-emerald-300 dark:border-emerald-700"/>
                    ...
                    {absenLoading && <div className="flex items-center gap-2 text-sm text-emerald-600"><Loader size={14} className="animate-spin"/> Memproses QR...</div>}
                  </div>
                )}
                ...
                <AnimatePresence>
                  {absenResult && (
                    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}}
                      className={`mt-4 p-4 rounded-2xl border flex items-start gap-3 ${absenResult.success ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' : 'bg-rose-50 dark:bg-rose-900/20 border-rose-200 dark:border-rose-800'}`}>
                      {absenResult.success ? <CheckCircle size={18} className="text-emerald-500 flex-shrink-0 mt-0.5"/> : <AlertCircle size={18} className="text-rose-500 flex-shrink-0 mt-0.5"/>}
                      <div className="flex-1 min-w-0">
                        ...
                        {absenResult.success && absenResult.data && (
                          <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2">
                            {[
                              {label:'Nama',       val: absenResult.data.siswa?.nama},
                              {label:'Kelas',      val: absenResult.data.siswa?.kelas},
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Cari nama siswa, NIS, NISN..."
                      ...
                      className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"/>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    <select value={filters.bulan} onChange={e => handleFilter('bulan', e.target.value)}
                      className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500">
                    ...
                    <button onClick={() => setShowFilters(!showFilters)}
                      className={`px-3 py-2 rounded-xl flex items-center gap-1.5 text-xs font-semibold border transition-all ${showFilters ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700'}`}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                          ...
                          <select value={filters.status_pulang} onChange={e => handleFilter('status_pulang', e.target.value)}
                            className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500">
          ...
          <motion.div key="statistik" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="space-y-4">
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-5">
                <div className="p-2 bg-emerald-100 dark:bg-emerald-900/40 rounded-xl"><BarChart3 size={16} className="text-emerald-600 dark:text-emerald-400"/></div>
                <div>
                    ...
                    <div key={idx} className="flex flex-col items-center gap-1">
                      <div className="w-full h-24 flex items-end gap-0.5">
                        <motion.div initial={{height:0}} animate={{height:`${hMasuk}%`}} transition={{delay:idx*0.05}}
                          className="flex-1 bg-emerald-300/60 dark:bg-emerald-500/40 rounded-t-md" title={`Masuk: ${item.total_masuk}`}/>
                        <motion.div initial={{height:0}} animate={{height:`${hPulang}%`}} transition={{delay:idx*0.05+0.1}}
                          className="flex-1 bg-teal-400/80 dark:bg-teal-500/60 rounded-t-md" title={`Pulang: ${item.total_pulang}`}/>
                      </div>
              ...
              <div className="flex items-center gap-4 mt-3 justify-center">
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-emerald-300/60"/><span className="text-[11px] text-slate-500">Masuk</span></div>
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-teal-400/80"/><span className="text-[11px] text-slate-500">Pulang</span></div>
              </div>
            </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800 rounded-2xl p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">{statistik.siswa_belum_pulang.length} siswa sudah masuk tapi belum absen pulang</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {statistik.siswa_belum_pulang.map((s,i) => (
                    <div key={i} className="flex items-center gap-2.5 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-800">
                      {s.foto ? <img src={s.foto} alt="" className="w-8 h-8 rounded-full object-cover flex-shrink-0"/>
                        : <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white font-bold text-xs flex-shrink-0">{s.nama.charAt(0)}</div>}
                      <div className="min-w-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                {label:'Total Sudah Pulang', val:statistik.total_sudah_pulang, sub:'Bulan ini', color:'from-emerald-500 to-teal-600'},
                {label:'Rata-rata Pulang Cepat', val:`${statistik.rata_cepat} mnt`, sub:'Lebih awal dari jadwal', color:'from-amber-500 to-orange-500'},
                {label:'Rata-rata Lembur', val:`${statistik.rata_lembur} mnt`, sub:'Setelah jam pulang', color:'from-teal-500 to-cyan-600'},
              ].map((s,i) => (
