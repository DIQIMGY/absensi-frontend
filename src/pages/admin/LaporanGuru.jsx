...
export default function LaporanGuru() {
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      console.log('Fetching laporan guru with params:', params)
      const response = await adminApi.getLaporanGuruBulanan(params)
      ...
      setStats({ total_absensi: totalAbsensi, hadir, terlambat, alpha })
    } catch (error) {
      console.error('Error fetching preview:', error)
      console.error('Error response:', error.response)
      toast.error(error.response?.data?.message || 'Gagal memuat data preview')
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (filters.format === 'pdf') {
        response = await adminApi.exportGuruPdf(params)
        fileName = `Laporan-Absensi-Guru-${params.start_date}-sd-${params.end_date}.pdf`
        mimeType = 'application/pdf'
      } else {
        response = await adminApi.exportGuruExcel(params)
        fileName = `Laporan-Absensi-Guru-${params.start_date}-sd-${params.end_date}.xlsx`
    ...
    <div className="w-full max-w-full overflow-x-hidden space-y-4 sm:space-y-6 px-2 sm:px-0">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5 truncate">
              Export dan analisis data absensi guru
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg sm:rounded-xl md:rounded-2xl p-2 sm:p-3 md:p-4 lg:p-5 border border-amber-200 dark:border-amber-800">
          <div className="flex items-center justify-between mb-1 sm:mb-2">
            <p className="text-[10px] sm:text-xs text-amber-600 dark:text-amber-400 truncate">Terlambat</p>
            <div className="p-1 sm:p-1.5 md:p-2 bg-white dark:bg-slate-800 rounded-lg flex-shrink-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <h3 className="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">
                Preview Data {previewData && `(${previewData.length} Guru)`}
              </h3>
            </div>
            <button
                    ...
                    <table className="w-full min-w-[800px] sm:min-w-full">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-700/50">
                          ...
                          <th className="text-left py-2 sm:py-3 px-2 sm:px-3 md:px-4 text-[10px] sm:text-xs font-semibold text-slate-600 dark:text-slate-300">NIP</th>
                          ...
                          <th className="text-left py-2 sm:py-3 px-2 sm:px-3 md:px-4 text-[10px] sm:text-xs font-semibold text-slate-600 dark:text-slate-300">Mata Pelajaran</th>
                          <th className="text-center py-2 sm:py-3 px-1 sm:px-2 text-[10px] sm:text-xs font-semibold text-emerald-600">Hadir</th>
                          <th className="text-center py-2 sm:py-3 px-1 sm:px-2 text-[10px] sm:text-xs font-semibold text-amber-600">Terlambat</th>
                          <th className="text-center py-2 sm:py-3 px-1 sm:px-2 text-[10px] sm:text-xs font-semibold text-rose-600">Alpha</th>
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {previewData.slice(0, 10).map((item, index) => (
                          ...
                          >
                            ...
                            <td className="py-2 sm:py-3 px-2 sm:px-3 md:px-4 text-[10px] sm:text-xs font-mono text-slate-900 dark:text-white truncate max-w-[60px] sm:max-w-none">{item.nip}</td>
                            ...
                            <td className="py-2 sm:py-3 px-2 sm:px-3 md:px-4 text-[10px] sm:text-xs text-slate-600 dark:text-slate-400 truncate max-w-[60px] sm:max-w-[80px]">{item.mata_pelajaran || '-'}</td>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-4 text-[10px] sm:text-xs text-teal-800 dark:text-teal-400">
              <ul className="space-y-1 sm:space-y-2">
                <li className="flex items-start gap-1 sm:gap-2">
                  ...
                  <span className="truncate">Laporan mencakup data absensi guru dari tanggal yang dipilih</span>
                </li>
                <li className="flex items-start gap-1 sm:gap-2">
                  <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-teal-500 mt-1 sm:mt-1.5 flex-shrink-0"></span>
                  <span className="truncate">Data harian yang terupdate secara real-time</span>
                </li>
                <li className="flex items-start gap-1 sm:gap-2">
                  <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-teal-500 mt-1 sm:mt-1.5 flex-shrink-0"></span>
