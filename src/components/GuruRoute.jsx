 ...
 *   hideForAbsensiOnly (default false) — block guru_mapel & karyawan (halaman absensi siswa)
 */
...
export default function GuruRoute({ children, requireFullAccess = true, kepsekOnly = false, hideForAbsensiOnly = false }) {
  ...
  // Halaman absensi siswa — tidak untuk guru_mapel & karyawan
  if (hideForAbsensiOnly && isAbsensiOnly) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 20 }}
        ...
        >
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-6">
            Fitur absensi siswa hanya dapat diakses oleh Kepala Sekolah dan Wali Kelas.
          </p>
          <button
            onClick={() => navigate('/guru/dashboard')}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition-colors shadow-sm"
