 ...
 * Desain: mirip laporan PDF — header identitas, ringkasan statistik, tabel harian
 */
import { useState, useEffect, useCallback } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import {
  X, Calendar, ChevronLeft, ChevronRight, RefreshCw,
  SlidersHorizontal, CalendarDays, User, GraduationCap,
  Hash, BookOpen, Printer, FileText,
} from 'lucide-react'
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      if (filterMode === 'range') {
        ...
        params.bulan = bulan; params.tahun = tahun
      }
      const res = isSiswa
        ...
        <motion.div
          ...
          <motion.div
                ...
                <div className="flex-1 flex items-center gap-2">
                  {filterMode === 'bulan' ? (
                    <div className="flex items-center gap-2 flex-1">
                      <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex-shrink-0">
                      ...
                      <button onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex-shrink-0">
                    ...
                    <div className="p-4 sm:p-5 flex items-center gap-4">
                      ...
                      {person?.foto ? (
                        <img src={person.foto} alt={person.nama}
                          className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl object-cover border-2 border-white dark:border-slate-700 shadow-md flex-shrink-0"/>
                      ) : (
                          ...
                          {isSiswa ? (
                            <>
                              <span className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                                ...
                                NIS: <strong className="text-slate-700 dark:text-slate-200">{person?.nis ?? '-'}</strong>
                              </span>
                              <span className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                                <GraduationCap size={11} className="text-slate-400 flex-shrink-0"/>
                                Kelas: <strong className="text-slate-700 dark:text-slate-200">{person?.kelas ?? '-'}</strong>
                              </span>
                            </>
                          ) : (
                            <>
                              <span className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                                <Hash size={11} className="text-slate-400 flex-shrink-0"/>
                                NIP: <strong className="text-slate-700 dark:text-slate-200">{person?.nip ?? '-'}</strong>
                              </span>
                              <span className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                                <BookOpen size={11} className="text-slate-400 flex-shrink-0"/>
                                Mapel: <strong className="text-slate-700 dark:text-slate-200">{person?.mata_pelajaran ?? '-'}</strong>
                              </span>
                              <span className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                                <User size={11} className="text-slate-400 flex-shrink-0"/>
                                Jabatan: <strong className="text-slate-700 dark:text-slate-200">{person?.jabatan ?? '-'}</strong>
                              </span>
                            </>
                          )}
                        </div>
                        <div className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs text-slate-500 dark:text-slate-400">
                    ...
                    {records.length === 0 ? (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs" style={{ minWidth: isSiswa ? '700px' : '620px' }}>
                          <thead>
                            <tr className={isSiswa
                              ? 'bg-teal-600 text-white'
                              : 'bg-indigo-600 text-white'}>
                              <th className="px-3 py-3 text-center font-semibold border-r border-white/20 w-10">No</th>
                              <th className="px-3 py-3 text-left font-semibold border-r border-white/20">Tanggal</th>
                              <th className="px-3 py-3 text-left font-semibold border-r border-white/20">Hari</th>
                              <th className="px-3 py-3 text-center font-semibold border-r border-white/20">Status</th>
                              <th className="px-3 py-3 text-center font-semibold border-r border-white/20">Masuk</th>
                              <th className="px-3 py-3 text-center font-semibold border-r border-white/20">Pulang</th>
                              <th className="px-3 py-3 text-center font-semibold border-r border-white/20">Terlambat</th>
                              <th className="px-3 py-3 text-left font-semibold border-r border-white/20">Keterangan</th>
                              <th className="px-3 py-3 text-center font-semibold">Metode</th>
                            </tr>
                          </thead>
                          <tbody>
                            {records.map((rec, i) => {
                              const sCfg = STATUS[rec.status] || STATUS.alpha
                              const isEven = i % 2 === 0
                              return (
                                <tr key={rec.id ?? i}
                                  className={`border-b border-slate-100 dark:border-slate-800 transition-colors hover:bg-teal-50/60 dark:hover:bg-teal-900/10 ${isEven ? 'bg-white dark:bg-slate-900' : 'bg-slate-50/70 dark:bg-slate-800/30'}`}>
                                  <td className="px-3 py-2.5 text-center text-slate-400 font-mono border-r border-slate-100 dark:border-slate-800">{i+1}</td>
                                  <td className="px-3 py-2.5 font-mono text-slate-700 dark:text-slate-300 whitespace-nowrap border-r border-slate-100 dark:border-slate-800">
                                    {rec.tanggal}
                                  </td>
                                  <td className="px-3 py-2.5 text-slate-600 dark:text-slate-400 whitespace-nowrap border-r border-slate-100 dark:border-slate-800">
                                    {rec.hari}
                                  </td>
                                  <td className="px-3 py-2.5 text-center border-r border-slate-100 dark:border-slate-800">
                                    <span className={`inline-block px-2 py-0.5 rounded-md ${sCfg.cls}`}>
                                      {sCfg.label}
                                    </span>
                                  </td>
                          ...
                          {/* footer summary */}
                          <tfoot>
                            <tr className="bg-slate-100 dark:bg-slate-800 font-bold text-xs border-t-2 border-slate-300 dark:border-slate-600">
                              <td colSpan={3} className="px-3 py-2.5 text-slate-600 dark:text-slate-300 text-right">Total</td>
                              <td className="px-3 py-2.5 text-center">
                                <span className="text-slate-700 dark:text-slate-200">{records.length} hari</span>
                              </td>
                              <td colSpan={2} className="px-3 py-2.5" />
                              <td className="px-3 py-2.5 text-center text-amber-600">
                                {records.filter(r=>r.menit_keterlambatan>0).length > 0
                                  ? `${records.filter(r=>r.menit_keterlambatan>0).length}x`
                                  : '—'
                                }
                              </td>
                              <td colSpan={2} className="px-3 py-2.5" />
            ...
            <div className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex-shrink-0">
              ...
              <button onClick={onClose}
                className="px-4 py-2 text-xs font-semibold border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 transition-colors flex-shrink-0">
                Tutup
              </button>
            </div>
